...
* Added per-brand audience priorities, reasons to follow and recognizable recurring series. These priorities are labelled working hypotheses. Six existing topics per owner now use a relevant follow CTA; existing keyword materials and other engagement directions remain. Fixed the two Veronika Threads card titles that previously displayed only `Коммерция`.
* Added `deploy/personal_brand_growth.py`: every source item carries an audience/need, exploratory creative hypothesis and decision signals. Replaced generic stored KPIs with each item's primary signal. An existing first-article row carries the batch strategy; no extra publication records were created.
* Added the reusable read-only `media_plan_growth.py` renderer. Both native calendars have a collapsed strategy/readiness section and each dialog a collapsed audience/measurement section. Day 7/14/21/30 review dates derive from the actual selected start. Includes production quality gates, genuine community-response responsibilities, unpublished profile-copy drafts, manual review ownership, platform-specific measurement and rules for revising only unprepared future topics.
* The analytics skill informed equal-age/platform comparisons, decision-oriented signals and missing-data handling. This implementation does not install tracking pixels, collect new personal data, enable an analytics integration or schedule automatic reports. Source review used Meta's Instagram/Threads guidance and YouTube retention documentation; no agency ranking or guaranteed-performance claim is made.
* Read-only production audit: sites 17/19 have zero social connections and zero stored social posts. Actual profile readiness and native account analytics remain unverified; the calendar explicitly distinguishes this from prepared editorial content. No generation, publication, outbound comment, integration or automation was triggered.
* Migration updated 226 pending rows, removed zero rows, and retained IDs, publication/production/recording times, statuses and unrelated/completed records. DB recovery: `/var/backups/blog-core/pre-personal-brand-launch-20260913-123253-300656.sqlite3`; source recovery: `/var/backups/blog-core/pre-growth-source-20260913`.
* Verification: nine regression tests passed locally and on the VPS, including completed/bound-content protection, idempotence, audience classification, escaping and review-date shifts. A production-copy migration passed before live application; the live second dry run changed zero rows. Both native routes return 200/noindex with 248 destination cards and 248 growth-detail sections each. Karp desktop and Veronika mobile browser checks confirmed accessible expansion, updated Reel content and no horizontal overflow or console errors. Both Blog Core PM2 processes restarted and health returned ok.

## 2026-09-13 — Assess launch-plan readiness against published social-marketing practices

* Compared the deployed plan's source contracts with published SocialChain, We Are Social, Meta and YouTube guidance. The approved format mix is a launch foundation, not evidence of agency-best performance or guaranteed growth.
* Verified that some attentionAngle fields remain general directions (for example, starting with a pleasant moment or the strongest property feature), rather than a specific creative reason for an unfamiliar viewer to stay. Internal topic titles need not be public hooks; the owner's natural-speech requirement remains unchanged.
* The authored launch-calendar contract contains generic KPI language, but no per-test hypothesis, accountable review cadence or result-based rule for revising remaining content. This observation is limited to the calendar and does not claim the rest of Blog Core lacks analytics.
* Identified additional readiness checks: prioritized audience/positioning, profile-to-follow/contact path, and an owned process for genuine replies and community participation. Actual social-account analytics and profile readiness were not inspected in this assessment.
* No production tasks, quotas, schedules, integrations or publication settings were changed. These findings are recommendations, not authorization to create extra work or resume first-month Stories/Q&A.
