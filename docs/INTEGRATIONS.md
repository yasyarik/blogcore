...
* Current strict social text limits: LinkedIn 3000 chars, Telegram 4096 chars, X/Twitter 280 chars, Tumblr 4096 chars, Pinterest description 500 chars. Drafts are normalized and shortened before storage if Gemini returns over-limit text, so stored `social_posts.char_count` must be `<= max_chars`.
