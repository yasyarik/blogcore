...
* My UGC Studio binding: `content-factory` at `http://127.0.0.1:3099`, with native blog URLs under `/blog/{slug}.html`. The source factory remains responsible for multilingual generation, its own static output, and publishing. It requests 1,800-2,200 rendered words and validates a 1,500-word body minimum, stages one hero plus three inline assets, and uses the current MyUGC shared header/footer from the live blog index. It passes the real logo as a multimodal image reference only for interface/product-screen visuals; ordinary editorial imagery remains logo-free.
...
* Zernio Pinterest publication maps the reviewed Pin's `pinTitle`, `destinationUrl`, and the configured `pinterest_board_id` into Pinterest platform-specific data. A board and a real HTTPS destination link are required for traffic-oriented Pins.
* Instagram drafts store a native carousel creative in `social_posts.content_json.instagramCarousel`: one shared carousel caption and exactly 6-8 substantive slides, each with a headline/subtext/image prompt/alt text for image generation/review. The text model receives an enforced JSON schema and one retry; failure creates no generic fallback draft. Slide 1 is a truthful, audience-specific scroll-stopping hook, not a repeated article H1 or generic clickbait. A carousel has one structured primary visual treatment plus a shared art-direction brief; cover and closing slides use it, while no more than two supporting graphics may explain a real framework/comparison. Generated slides use `imageMimeType=image/jpeg` and `visualSpec.aspectRatio=4:5`. Gemini Image receives any scanned raster logo through its canonical `generateContent` image-reference fields (`inlineData` and `mimeType`); `visualSpec.logoReferenceProvided` and its non-secret source path record what was attached. Gemini decides independently for every slide whether the reference is materially relevant; the default is no logo, and Blog Core never force-inserts or overlays one. Each draft receives an immutable random asset key, so regenerating the same job cannot overwrite a submitted carousel. Generated JPEG slide files are written under ignored `data/social_assets/{site_id}/{asset_key}/instagram/` and served by `/sites/{site_id}/social-assets/{asset_key}/instagram/{filename}`. The review page is `/sites/{site_id}/social-posts/{post_id}/instagram-carousel`.
* For local-webroot sites without an image in the scanned header, logo lookup ranks source-owned `assets/brand`/branding assets above root fallback logos and excludes favicons and extension assets. The resolved source is runtime metadata only and is not stored as a secret.
...
* Threads is configured through the per-site Zernio mapping. Threads drafts select `question`, `observation`, `contrarian`, `micro_story`, or `objection_answer`; they remain short, conversational, and non-promotional with at most one hashtag. Media is one natural 4:5 JPEG with no overlay text/logo/UI screenshot under ignored `data/social_assets/{site_id}/{asset_key}/threads/image-01.jpg`. Every media-generating social draft uses its own immutable asset key. The review page is `/sites/{site_id}/social-posts/{post_id}/threads`.
* Explicit `POST /api/sites/{site_id}/content-jobs/{job_id}/social-publish/zernio` submits ready Zernio-channel drafts. It is intentionally separate from draft generation and does not send anything without an operator action. An optional `socialPostIds` list targets particular reviewed drafts when several variants exist for one content task. Account mappings, a Pinterest board, and a Reddit subreddit must be present where relevant.
...
* Pinterest also supports standalone visual showcase Pins in `visual_pins`. They are not article derivatives: Blog Core uses the text model to choose a fresh original visual concept, creates one complete 2:3 JPEG collage through Gemini Image, and keeps the result as a reviewable `DRAFT`. The optional visual-story modes express one garment across many people/locations, one model across many looks, or one concept across scenes. No manual clothing/product reference is needed and generated concepts must not copy real brands or campaigns. All product-variation Pins use one visual contract: a human-free source product above a horizontal divider, with three or four varied model/location applications of that exact product below.
* When a scanned site exposes a raster logo, a visual showcase Pin passes that actual asset to Gemini as the brand reference and composites the same asset after image generation in a restrained top-corner badge. Gemini is explicitly forbidden from drawing a logo, text, UI, price claim, or badge. The Pin description, rather than generated text baked into the image, explains the connected product capability.
* Visual showcase Pins can be published manually through the Zernio Pinterest mapping or picked up by the existing reviewed-Pinterest cadence. They require the same per-site Pinterest account and board mapping as article Pins; they do not publish a site page or alter a blog template.
...
* Social scheduling is independent per channel in `autopublish_settings.social_cadences_json`: each connected Zernio channel, plus a fully OAuth-connected LinkedIn account, can be paused (`0`) or set to 1-12 posts per day within the site's distribution window/timezone. The shared scheduler never creates or publishes an article/page. At a due slot it sends the oldest existing channel `DRAFT`; if none exists, it selects the oldest published article with no prior non-error post for that channel, generates the native creative/media, and submits it.
* Zernio is the source of truth for social delivery status. Blog Core reconciles each local draft against recent Zernio posts by the complete immutable set of generated media URLs, including after a request exception. Provider `published` and `scheduled` states replace stale local `ERROR`/`SUBMITTED` states; reconciliation never sends a post.
* LinkedIn publishing uses `POST https://api.linkedin.com/rest/posts` with the per-site OAuth member/organization author URN. The app credentials only start OAuth; they do not make a site publishable. A completed connection is required before a LinkedIn draft can be sent manually or by cadence.
* A LinkedIn Company Page uses `urn:li:organization:…`, not the connected member's `urn:li:person:…`. The OAuth app must request `w_organization_social`, and that member needs an approved organization role. Client ID and Client Secret remain only in server environment configuration and must never be copied into per-site records or documentation.
