...
* Threads is configured as a separate per-site provider with `access_token` and optional `threads_user_id`. Threads drafts use the generic `social_posts` text draft path with Threads-specific byte validation and at most one hashtag.
