...
* Instagram drafts store a native carousel creative in `social_posts.content_json.instagramCarousel`: one shared carousel caption and exactly 6-8 substantive slides, each with a headline/subtext/image prompt/alt text for image generation/review. The text model receives an enforced JSON schema and one retry; failure creates no generic fallback draft. Generated slides use `imageMimeType=image/jpeg` and `visualSpec.aspectRatio=4:5`. Gemini Image is called through the Gemini Interactions API with `GEMINI_API_KEY` or `GOOGLE_API_KEY`; optional `GEMINI_IMAGE_MODEL` can override the default image model. Where a scanned raster logo is available, every slide receives it as a Gemini reference and Blog Core composites that exact logo afterward, rather than trusting model-drawn branding. Generated JPEG slide files are written under ignored `data/social_assets/{site_id}/{job_id}/instagram/` and served by `/sites/{site_id}/social-assets/{job_id}/instagram/{filename}`. The review page is `/sites/{site_id}/social-posts/{post_id}/instagram-carousel`.
