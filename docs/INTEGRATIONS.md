...
* Social connection tests currently use provider API probes: Telegram `getMe/getChat`, LinkedIn `userinfo`, X `/2/users/me`, Tumblr OAuth 1.0 `user/info`, Pinterest API v5 `user_account`, and Instagram Graph `/{instagram_user_id}?fields=id,username`.
...
* Current strict social text limits: LinkedIn 3000 chars, Telegram 4096 chars, X/Twitter 280 chars, Tumblr 4096 chars, Pinterest description 500 chars, and Instagram caption 2200 chars. Drafts are normalized and shortened before storage if Gemini returns over-limit text, so stored `social_posts.char_count` must be `<= max_chars`.
