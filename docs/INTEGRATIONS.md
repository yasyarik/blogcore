...
* Social connection tests currently use provider API probes: Telegram `getMe/getChat`, LinkedIn `userinfo`, X `/2/users/me`, Tumblr OAuth 1.0 `user/info`, and Pinterest API v5 `user_account`. Instagram stores third-party intermediary credentials; a full publish/connect probe needs the intermediary API contract.
