...
* Social scheduling is independent per channel in `autopublish_settings.social_cadences_json`: each connected Zernio channel, plus a fully OAuth-connected LinkedIn account, can be paused (`0`) or set to 1-12 reviewed drafts per day within the site's distribution window/timezone. The shared scheduler never creates an article, generates new media, or publishes an unreviewed social draft; it only sends the oldest reviewed `DRAFT` for the due channel slot.
