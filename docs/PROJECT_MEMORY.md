...
* Threads is a separate social channel from X/Twitter and Instagram. Threads text drafts must stay within 500 UTF-8 bytes and use at most one hashtag; Blog Core should validate bytes, not only Python character count, because emoji/non-ASCII text can exceed platform limits sooner.
