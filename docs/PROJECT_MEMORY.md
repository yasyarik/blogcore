...
* Current production state: On 2026-07-04, `airep24.com` site `id=9` was connected to Blog Core with `root_path=/var/www/airep24.com` and `access_type=local_path`, then fully migrated from `/var/www/content-factory-airep24/factory.sqlite`. Blog Core now has 80 AIREP24 `content_jobs`: 24 `IMPORTED` inventory records and 56 `QUEUED` planned jobs. The imported inventory contains the original 16 English blog/webroot records plus 8 legacy published factory records: 4 localized home pages and 4 localized `features/automated-knowledge-base` SEO money pages. The planned queue contains 20 blog jobs and 36 SEO money-page jobs across EN/DE/ES/FR, with `sources_json.migratedFrom=content-factory-airep24`, `oldFactoryJobId`, `contentType`, `pageKind`, `language`, `targetPath`, and `canonicalGroup` preserved.
