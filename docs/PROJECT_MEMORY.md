...
* Threads is a separate social channel from X/Twitter and Instagram. Threads drafts must feel native to Threads: short, conversational, question-led or opinion-led, not promotional ad copy. They must stay within 500 UTF-8 bytes, use at most one hashtag, and may attach one relevant image from existing generated social assets. Blog Core should validate bytes, not only Python character count, because emoji/non-ASCII text can exceed platform limits sooner.
