...
* Replaced/deprecated 2026-08-14: A single new TTS call for the complete script plus scene durations inferred from paragraph word counts does not preserve semantic alignment at scene boundaries and must not be used.
* Current rule: each approved scene owns one narration sentence that expresses the same idea as that scene's overlay, with fuller spoken detail allowed. TTS renders that locked sentence once for its scene; the renderer then assembles all existing scene clips and scene-aligned silence into one continuous `reel-narration.wav` before the final FFmpeg mix.
* The final render receives one continuous narration input. Scene boundaries never restart voice, and narration for one scene cannot spill into or be reassigned to another scene through estimated word-weight timing.
