...
* Discovery article idea generation should scale with selected signal volume. For many selected signals, the journalist/SEO generator should target a larger set of ideas, run a second pass if validated ideas are below target, and show accepted/target/generated/rejected counts instead of silently returning a tiny list.
