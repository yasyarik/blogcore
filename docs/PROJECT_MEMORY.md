...
* Current production state: On 2026-07-04, `airep24.com` site `id=9` was connected to Blog Core with `root_path=/var/www/airep24.com` and `access_type=local_path`. Blog Core imported 16 existing English blog URLs directly from the local VPS webroot with `sources_json.webrootPath` pointing to `/var/www/airep24.com/...`: 15 article pages plus the `/blog/` hub metadata record. The Content inventory hides the hub, so 15 imported live article records are visible.
