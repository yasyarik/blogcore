...
* Replaced/deprecated 2026-08-14: independent TTS calls per scene are unsafe with Gemini TTS because the model can speak the delivery instruction instead of the scene narration. They must not be treated as correct merely because their WAV hashes or durations differ.
* Current rule: each approved scene still owns one locked narration sentence expressing the same idea as its overlay, with fuller spoken detail allowed, but all sentences are synthesized in one ordered TTS request. The generated source WAV is automatically divided only at measured audio pauses between paragraphs and each unchanged segment is padded into its owning planned scene before one aligned `reel-narration.wav` is written.
* The pause detector must find exactly `scene_count - 1` usable internal boundaries. If it cannot, production stops before rendering; paragraph boundaries must never be estimated from word counts or equal proportions.
* The final render receives one continuous aligned narration input. Scene boundaries never restart voice, and one scene's sentence cannot spill into another scene.
* Natural voice duration controls each scene's minimum duration. Existing approved narration and its established timeline are reused byte-for-byte during visual-only corrections; they are not regenerated, accelerated, shortened, or replaced without an explicit voice change request.
