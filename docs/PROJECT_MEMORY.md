...
* Updated 2026-09-13: the initial 30 generic owner-Reel briefs per person are superseded by 60 individually authored scripts (30 main at the retained 19:00 slot, 30 short at 12:30 Europe/Warsaw). Each is personally recorded and cross-published to Instagram, TikTok and YouTube Shorts, yielding 120 source tasks / 255 destination cards per site with existing factory tasks unchanged. Two daily Reels are a test cadence, not a platform requirement.
* Owner briefs must include actual opening dialogue, complete timed speech through the payoff and one CTA, concrete shots and a reason to keep watching. Do not derive personal-Reel scripts from stock sentence templates with inserted article questions. Aleksei covers apartments/new builds; Veronika covers houses/villas and commercial property. Include positive lifestyle and human humour, not only warnings.
* Keyword-comment CTAs may promise only prepared material. The 2026-09-13 library includes 14 usable text checklists/templates and ready-to-send manual replies inside the calendar dialog (31 of 120 Reels use them). No automatic DM/comment delivery is implied or enabled. See `docs/PERSONAL_BRAND_REELS.md`; authored source is `deploy/personal_brand_reels.py`. The targeted updater preserves main-row IDs, shifted dates, completion states and factory records, and is repeatable without duplicate shorts.
