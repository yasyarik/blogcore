...
* Every reviewed launch row has a `growth` contract (audience, need, recurring series, exploratory hypothesis and primary/secondary decision signals); the stored `kpi` matches its primary signal. One existing article row carries the batch `growthPlan`. `media_plan_growth.py` renders this data in collapsed strategy/brief sections without hardcoding brand-specific behaviour or creating publication tasks.
* Audience priorities are explicitly working hypotheses, not researched demographic claims: Karp prioritizes Kazan apartment buyers for their own life, with rental-investment needs secondary; Veronika prioritizes family homes, with villas and commercial property distinctly labelled. Existing source scopes and other site strategies are not overwritten.
* Growth review checkpoints are days 7/14/21/30 relative to the actual selected start, recomputed after rescheduling. This revision adds an accountable manual review protocol, not an automatic analytics collector/report scheduler. Compare equal-age publications within each platform/format, distinguish missing metrics from zero, label reach versus view denominators, and do not infer unique people or attribution across networks. Change only unprepared future content within agreed constraints after repeated evidence, not one viral outlier.
* Readiness audit on 2026-09-13 found no `social_connections` or `social_posts` rows for sites 17/19. Prepared calendars are not proof of active social publishing or analytics. Profile descriptions in the strategy are unpublished drafts; account-specific bio/link/pinning and permissions need verification after access. Do not silently connect providers, post comments, enable delivery, or add Stories/Q&A as part of a strategy revision.
* Updated 2026-09-13: the v2 batch of 60 timed scripts / 15 Telegram posts is superseded by the approved first-month launch plan: 60 owner-Reel topic briefs, 15 articles, 15 shared carousels, 15 Threads posts and 8 Telegram posts per person. This yields 113 source tasks / 248 destination cards per site. Existing Reel slots at 12:30 and 19:00 Europe/Warsaw are retained; crossposts do not add filming tasks. Two daily Reels are a test cadence, not a platform requirement.
* Superseded by the operator's 2026-09-13 clarification: the v2 requirement for opening dialogue, complete timed speech and prescribed shots is rejected. Live-owner Reels require a topic, direction, points to notice/show, an opening-interest angle and a suitable engagement direction, not literal dialogue or second-by-second scripts. Use recurring categories and alternate their topics through the calendar: lifestyle, property/developer/location reviews, lifehacks, things to check, comparisons, real work/cases and audience questions. Aleksei covers apartments/new builds; Veronika covers houses/villas and commercial property. Include positive lifestyle and personality, not only warnings. This is the editorial requirement; preparing a local library does not establish that a production calendar has been migrated.
* Mandatory distribution is not open for reduction: one owner Reel is the same production asset published to Instagram, TikTok and YouTube Shorts; one carousel is the same production asset published to Instagram and TikTok. Do not count crossposts as separate filming/design work or infer excessive workload from destination-card totals. The operator approved eight monthly Telegram posts on 2026-09-13, superseding the recommendation-only state. Strategy assessments alone still must not change schedules or create tasks.
* First-month priority is new-audience acquisition. Do not seed Stories or an audience-question response rubric before an audience and real questions exist. Viewing-day footage belongs in Reels. Revisit Stories and genuine audience Q&A after roughly the first month, relative to the chosen launch date, without automatically enabling either. The first 60-Reel mix is 10 lifestyle, 12 property reviews, 8 developer/location, 8 lifehacks, 8 things to check, 8 comparisons and 6 real-work/backstage briefs, distributed without adjacent repeated rubrics.
* First-month factory assets have distinct editorial jobs. Carousels carry seven meaningful slide messages, not seven chunks of the article. Threads has standalone planned copy without a forced article teaser or invented audience comments. Telegram has eight useful text/photo briefs at retained day offsets 1/5/9/13/17/21/25/29. Article themes and the 15-article quota are retained. Source-task revisions do not themselves generate assets, enable integrations or submit social publications.
* Keyword-comment CTAs may promise only prepared material. The library includes 14 usable text checklists/templates and ready-to-send manual replies inside the calendar dialog. Briefs explicitly instruct the owner to invite the viewer to write the keyword in a comment; this is a direction, not a mandatory spoken sentence. No automatic DM/comment delivery is implied or enabled. See `docs/PERSONAL_BRAND_REELS.md`. `deploy/update_personal_brand_launch_plan.py` revises only the two reviewed first-month batches, preserves retained IDs/dates/statuses and completed/unrelated rows, removes only the seven untouched surplus Telegram plans per site, backs up first and is idempotent.

* Superseded 2026-09-13: the calendar renderer is no longer limited to manually seeded Aleksei/Veronika batches. Every connected site has an automatic managed page at `/sites/{siteId}/media-plan`; the same renderer serves `/media-plan` on a site host whenever that native/hosted integration routes the path to Blog Core. The site dashboard and management page link to the managed calendar immediately after onboarding.
* The calendar reads every `agent_media_plan_items` row for the site on each request. A task with explicit `planMonth`/`publishAt` keeps that schedule; an undated task is grouped by its creation month and appears immediately under `Без даты`. Choosing a start date materializes missing publication dates from the task weeks and shifts already dated tasks as one batch. No seed script or page rebuild is required when tasks are added.
* Connected publishing destinations are also read live from `social_connections`. Direct connections and every mapped Zernio account automatically add their platform logo, filter and `completed/total` counter, including `0/0` before the first task. Cross-platform assignments fan out into one visible destination card per network. Supported calendar identities include website, Instagram, TikTok, YouTube, Telegram, Threads, LinkedIn, Facebook, X, Pinterest, Reddit and Tumblr; unknown/off-platform work remains visible as `Другие задачи`.
* New generic calendars derive their primary/accent colours and logo URL from the scanned site theme. The verified Karp and Veronika logo/palette overrides remain unchanged. All calendar variants retain noindex/no-store protections.
* Aleksei Karp and Veronika Veselova each have a separate site-scoped client calendar at `/media-plan` on their own domain. Nginx must proxy both the exact page and the `/media-plan/` action prefix to Blog Core; otherwise rescheduling and Reel-completion POSTs fall through to the native site and return 404. The route family must remain excluded from sitemaps, uncached, and protected by both a robots meta tag and `X-Robots-Tag: noindex, nofollow, noarchive`.
...
* Public client status vocabulary is deliberately short: `В плане`, `Готово`, `Ошибка`, `Просрочено`. Completed cards are visually faded. A completed factory-owned social card becomes a direct new-tab link only when the publication contour has stored a real public HTTP(S) destination URL; provider-side IDs and unconfirmed submissions must never be exposed as links. Personal-content cards remain detail/action cards. Errors use a bright red contour and shadow; overdue cards use a blinking orange contour and shadow. `Просрочено` is derived when the publication time has passed without a completed state. Personal live Reels have a one-way `Отметить готово` action; it advances the shared source row to `READY`, updates all three destination cards together, and cannot be cancelled from the client calendar.
