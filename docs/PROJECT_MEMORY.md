...
* Gemini JSON responses for article/page generation can occasionally be malformed even with JSON response mode. Blog Core should attempt a repair pass that asks Gemini to return valid JSON only before marking the job as failed.
