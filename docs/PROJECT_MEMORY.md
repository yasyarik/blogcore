...
- Current behavior: the factory requests Gemini JSON with a response schema, performs research grounding, retries validation, generates a hero plus three inline WebP images, writes EN plus enabled locale pages, updates the blog indexes/sitemaps, and commits only changed landing assets to the landing repository.
- Replaced quality gap: MyUGC now requests 1,800-2,200 visible body words and rejects a draft below 1,500 rendered words before preview or publish. Its existing structural checks remain: headings, metadata, table/list, 3 images, internal links, FAQ, and product context.
- Replaced native-style gap: `factory/landing.py` continues to render the site-owned article component from `landing/blog/template.html`, but each new preview/public page now takes the current shared-header CSS/JS and footer directly from `/var/www/landing/blog/index.html`. It must use the live shared MyUGC chrome rather than a stale copied header/footer.
- Replaced visual-identity gap: ordinary editorial photos stay logo-free. When a requested hero or inline image depicts an interface, dashboard, product screen, or browser extension, the factory sends `/var/www/landing/logo-ui.webp` to Gemini as an image input and composites the same real logo afterward; it must never use a generated approximation.
- Preview compatibility: Blog Core first asks a source factory for a private preview link. If a compatible legacy factory responds `404` because it exposes only native `/preview/{id}`, Blog Core proxies that native preview instead of falling back to generic HTML or reporting a false error.
...
- SEO priority: generic whole-yacht charter is a supporting, highly competitive topic rather than CabinJoin's principal organic territory. The first priority is (a) the solo/singles/cabin-charter search cluster for joining an organised trip and (b) the fragmented organiser workflow cluster for turning a secured charter into a public trip, recruiting a crew, defining terms, and managing requests. Do not represent Google Autocomplete phrases as measured search volumes; it confirms query wording, while Trends/Keyword Planner are required for numeric or country-level demand claims.
