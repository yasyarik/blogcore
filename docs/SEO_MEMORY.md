...
* Replaced 2026-07-26: the separate related/recommended utility presentation is now one localized commercial recommendation grid; FAQ remains a separate subsection below it. Factory internal links and descriptions are preserved and deduplicated. Supporting and recommendation imagery is allocated from a stable non-money-page asset partition so `/how-it-works`, `/coverage`, and `/pricing` share no main-content image within a language. Each page retains its own three validated factory figures. The former target-hero card imagery and page-local-only uniqueness rule are deprecated.
* Root money-page heroes are deliberately different scenes, not merely different URLs: workflow for How it works, an aerial geographic coverage landscape for Coverage, and a publishable widget/subscription scene for Pricing. The production audit rejects both repeated hero URLs and any cross-page main-image overlap.
...
* Factory storage still uses the `use_case` type, but `/use-cases/{slug}/` aliases for the three root pages permanently redirect to their root canonical and are excluded from the use-case hub and sitemap.
