...
* `georivo-gsc-submit.timer` now retries the official submission daily. Runtime state is written to ignored `data/georivo-gsc-status.json` as `blocked`, `error`, or `submitted`; only `submitted` with a returned API sitemap record closes this SEO gap.
