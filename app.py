...
async function createIdeasFromSignals(){const selected=currentSignals.slice();if(!selected.length){showToast('Deep analysis is not ready yet');return;}setGenerateIdeasEnabled(false);const box=document.getElementById('articleIdeaResult');box.hidden=false;box.className='loading idea-stage';box.textContent='Generating SEO-weighted editorial article ideas and checking existing site content...';showToast('Generating article ideas...');try{const res=await fetch('/api/sites/'+SITE_ID+'/article-ideas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({range:currentRange,signals:selected})});const data=await res.json();if(!res.ok) throw new Error(data.error||res.statusText);renderArticleIdeas(data.ideas||[],data.rejectedSimilar||[],data.counts||{});showToast('Article ideas ready: '+((data.ideas||[]).length)+' valid ideas');}catch(e){box.className='loading idea-stage';box.textContent='Article ideas failed: '+e.message;showToast('Article ideas failed: '+e.message);}finally{setGenerateIdeasEnabled(currentSignals.length>0);}}
